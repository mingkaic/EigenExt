...
TEST(OPS, Slice)
{
	...
	a.template slice<size_t>({1, 0, 0}, {3, 2, 2}).write(out0);
	std::vector<double> expect_data0 = { 9, 8, 1, 7, 2 };
	std::vector<size_t> expect_index0 = { 1, 2, 6, 7, 11 };
...
TEST(OPS, SliceIdentity)
{
	...
	std::vector<size_t> index0;
	SparseTensorDst<double,std::vector<double>> out0(nnz, data0, index0, vector_info<double>());
	a.template slice<size_t>({0, 0, 0}, {4, 3, 2}).write(out0);
